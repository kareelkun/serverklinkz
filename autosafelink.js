...
var cekantisafelink = cekantisafelink();
	...
	for (var i = 0; i < linktagMauDiganti.length; i++) {	
		...
		if (periksa == false) {
		var urlakandiENCODE = convertstr(linktagMauDiganti[i].href), envariabel = base64_encode(urlakandiENCODE+'||'+SETTINGKlinkz.TITLElink+'||'+SETTINGKlinkz.PASSlink+'||'+SETTINGKlinkz.LINKproperty+'||'+SETTINGKlinkz.VERIFYlink+'||'+SETTINGKlinkz.COUNTDOWNlink+'||'+SETTINGKlinkz.CLICK2Xlink+'||'), enkripsivalid = envariabel.replace('=','');
			linktagMauDiganti[i].href = SETTINGKlinkz.URLSAFELINK+'?kareeI='+enkripsivalid;
